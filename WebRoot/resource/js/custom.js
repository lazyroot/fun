...
$(document).ready(function() {
	...
	$('.toggle-collapsed').each(function(){
		if($(this).hasClass('on-hover')){
			...
			$(this).parent().hover(function(){
				...
				$(this).find('img').attr("src", 'resource/img/toggle-subnav-up-white.png');
			}, function(){
				$(this).removeClass("open");
				$(this).find('.collapsed-nav').slideUp();
				$(this).find('img').attr("src", 'resource/img/toggle-subnav-down.png');
			});
		} else {
	...
	if($('.dataTable').length > 0){
		$('.dataTable').each(function(){
			...
			if($(this).hasClass('dataTable-tools')){
				...
				opt.oTableTools = {
					"sSwfPath": "resource/js/tableTools/swf/copy_csv_xls_pdf.swf"
				};
			}
			$(this).dataTable(opt);
	...
	if($('.plupload').length > 0){
		$('.plupload').pluploadQueue({
			...
			url : 'resource/js/plupload/upload.php',
			max_file_size : '10mb',
			chunk_size : '1mb',
			unique_names : true,
			...
			silverlight_xap_url : 'resource/js/plupload/plupload.silverlight.xap'
		});
